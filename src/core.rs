...
use crate::traits::*;
use crate::{
    bigint, BigInt, DecryptionKey, EncryptionKey, Keypair, MinimalDecryptionKey,
    MinimalEncryptionKey, Paillier, RawCiphertext, RawPlaintext,
};
...
impl<'c, 'm> Decrypt<DecryptionKey, &'c RawCiphertext<'c>, RawPlaintext<'m>> for Paillier {
    fn decrypt(dk: &DecryptionKey, c: &'c RawCiphertext<'c>) -> RawPlaintext<'m> {
        ...
        let dk_pp = &dk.p * &dk.p;
        let dk_n = &dk.p * &dk.q;
        let dk_pinv = dk.p.invert(&dk.q).unwrap();
        ...
        let (mp, mq) = join(
            || {
                // process using p
                let dp = cp.powm(&dk_pminusone, &dk_pp);
                let lp = l(&dp, &dk.p);
                (&lp * &dk_hp) % &dk.p
            },
            || {
                // process using q
                let dq = cq.powm(&dk_qminusone, &dk_qq);
                let lq = l(&dq, &dk.q);
                (&lq * &dk_hq) % &dk.q
            },
        );
    ...
    for Paillier
{
    fn mul(ek: &EncryptionKey, c: RawCiphertext<'c>, m: RawPlaintext<'m>) -> RawCiphertext<'d> {
        RawCiphertext(Cow::Owned(
            (c.0.borrow() as &BigInt).powm(m.0.borrow(), &ek.nn),
        ))
    }
}
...
mod tests {
    ...
    fn test_correct_keygen() {
        let (ek, dk): (EncryptionKey, _) =
            Paillier::keypair_with_modulus_size(&mut rand::thread_rng(), 2048).keys();
