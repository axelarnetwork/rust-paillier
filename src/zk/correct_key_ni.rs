...
use crate::{extract_nroot, BigInt, DecryptionKey, EncryptionKey};
use rayon::prelude::*;
...
// salt string "axelar" as system parameter, which is hashed to 256 bit
const SALT_STRING: &[u8] = b"axelar";
const M2: usize = 11;
...
impl NICorrectKeyProof {
    ...
    pub fn verify(
        ...
        let salt_str = salt_str.unwrap_or(SALT_STRING);
        ...
        let derived_rho_vec = (0..M2)
            .into_par_iter()
            .map(|i| self.sigma_vec[i].powm(&ek.n, &ek.n))
            .collect::<Vec<BigInt>>();
...
mod tests {
    ...
    fn test_correct_zk_proof_no_salt_str() {
        ...
        assert!(proof.verify(&ek, None).is_ok());
    }

    #[test]
    fn test_correct_zk_proof_with_salt_str() {
        let salt_str: &[u8] = b"foobar";
        let (ek, dk) = Paillier::keypair(&mut rand::thread_rng()).keys();
        let proof = NICorrectKeyProof::proof(&dk, Some(salt_str));
        assert!(proof.verify(&ek, Some(salt_str)).is_ok());
    }
}
